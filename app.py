...
user_generate_model = {}
user_pending_photo = {}
user_video_mode = {}          # 'text' или 'image_one' или 'image_two'
user_video_frames = {}        # {'first': b64, 'last': b64 or None}
user_video_params = {}        # {'duration': 5, 'resolution': '480p', 'audio': True}
user_video_prompt = {}        # временное хранение промпта (сейчас не используется, т.к. промпт вводится после параметров)
...
# ================== 4. ГЕНЕРАЦИЯ ВИДЕО (С ПАРАМЕТРАМИ) ==================
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            if status == "completed":
                ...
                # 1. Пробуем unsigned_urls без Authorization
                ...
                # 2. Fallback: /content с Authorization
...
def generate_video_seedance_async(chat_id, prompt, first_frame_b64=None, last_frame_b64=None):
    """
    Запускает генерацию видео с параметрами из user_video_params.
    """
...
def video_params_keyboard(chat_id):
    """Генерирует инлайн-клавиатуру для выбора параметров видео."""
    params = user_video_params.get(chat_id, {})
    duration = params.get('duration', 5)
    resolution = params.get('resolution', '480p')
    audio = params.get('audio', True)
...
def start_video_param_selection(chat_id):
    """Отправляет сообщение с клавиатурой выбора параметров."""
...
def video_params_done(call):
    chat_id = call.message.chat.id
    bot.delete_message(chat_id, call.message.message_id)
    # Проверяем, все ли параметры выбраны (если нет, используем значения по умолчанию)
    params = user_video_params.get(chat_id, {})
...
def handle_video_prompt(message):
    ...
    # Не удаляем кадры сразу, они могут пригодиться для повторной попытки
