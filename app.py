...
VIDEO_MODEL_FEATURES = {
    ...
    'alibaba/wan-2.6': {'audio': True, 'resolution': True},
    'minimax/hailuo-2.3': {'audio': False, 'resolution': False},
...
def poll_video_task(polling_url, headers, chat_id, status_message_id, model_display=""):
    ...
    for attempt in range(1, 91):  # максимум 90 попыток × 10 сек = 15 мин
        time.sleep(10)
        try:
            ...
            progress = data.get("progress")  # может быть числом 0-100 или отсутствовать
            ...
            if progress is not None and isinstance(progress, (int, float)):
                text = f"🎬 Генерация видео ({model_display}): {int(progress)}%"
            else:
                text = f"🎬 Генерация видео ({model_display})... попытка {attempt}"
            try:
            ...
            if status == "completed":
                ...
                # Запасной путь: /videos/{job_id}/content
...
def generate_video_seedance_async(chat_id, prompt, first_frame_b64=None, last_frame_b64=None):
    ...
    model_id = user_video_model.get(chat_id, 'alibaba/wan-2.6')  # по умолчанию Wan
...
def set_video_model(call):
    ...
    model_key = call.data.split('_', 1)[1]  # seedance-2.0 и т.д.
    model_map = {
